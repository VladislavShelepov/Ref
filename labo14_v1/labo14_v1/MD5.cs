...
namespace labo14_v1
{
	public class MD5
	{
		...
		public string Value
		{
			...
			{
				string st;
				char[] tempCharArray = new Char[inputByteArray.Length];

				for (int i = 0; i < inputByteArray.Length; i++)
					tempCharArray[i] = (char)inputByteArray[i];
			...
			set
			{
				...
				inputByteArray = new byte[value.Length];
				for (int i = 0; i < value.Length; i++)
					inputByteArray[i] = (byte)value[i];
				myHash = CalculateMD5Value();

				/// raise the event to notify the change
				if (this.OnValueChanged != null)
					this.OnValueChanged(this, new MD5ChangedEventArgs(value, myHash.ToString()));
		...
		public byte[] ValueAsByte
		{
			get
			{
				byte[] bt = new byte[inputByteArray.Length];
				for (int i = 0; i < inputByteArray.Length; i++)
					bt[i] = inputByteArray[i];
				return bt;
			}
			set
			{
		...
		protected byte[] CreatePaddedBuffer()
		{
			...
			int temp = (448 - ((inputByteArray.Length * 8) % 512)); //temporary 
			...
			sizeMsgBuff = (uint)((inputByteArray.Length) + (pad / 8) + 8);
			sizeMsg = (ulong)inputByteArray.Length * 8;
			bMsg = new byte[sizeMsgBuff];   //no need to pad with 0 coz new bytes 
											// are already initialize to 0 :)

			////copying string to buffer 
			for (int i = 0; i < inputByteArray.Length; i++)
				bMsg[i] = inputByteArray[i];

			bMsg[inputByteArray.Length] |= 0x80;        //making first bit of padding 1,
